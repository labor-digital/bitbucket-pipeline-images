...
out=$(echo $out|tr -d '\n'|tr -d '\r')
echo "Running command: build -f $BUILD_AND_PUSH_DOCKER_FILE -t $BUILD_AND_PUSH_IMAGE $out ."
docker build -f $BUILD_AND_PUSH_DOCKER_FILE -t $BUILD_AND_PUSH_IMAGE $out .
